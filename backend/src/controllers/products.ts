...
const getProducts = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const offsetByParam = Math.max(parseInt(String(req.query.offset ?? '0'), 10), 0);
...
const updateProduct = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const product = await Product.findByIdAndUpdate(productId, { $set }, { runValidators: true, new: true }).orFail(
